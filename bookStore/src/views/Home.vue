<template>
    <div class="bg-pink-50 p-3 overflow-auto h-screen" @scroll="handleScroll" ref="scrollContainer" >
        <!-- 搜索框 -->
        <van-sticky>
...
function fetchMoreBooks(page) {
    return new Promise((resolve) => {
        setTimeout(() => {
            const newBooks = [
                { id: page * 10 + 1, title: 'Book ' + (page * 10 + 1), image: 'https://via.placeholder.com/150' },
                { id: page * 10 + 2, title: 'Book ' + (page * 10 + 2), image: 'https://via.placeholder.com/150' },
                { id: page * 10 + 3, title: 'Book ' + (page * 10 + 3), image: 'https://via.placeholder.com/150' },
            ];
            resolve(newBooks);
        }, 1000); // 模拟网络延迟
    });
...
async function loadMoreBooks() {
    loading.value = true;
    const newBooks = await fetchMoreBooks(currentPage++);
    bookCommendState.value.push(...newBooks);
    loading.value = false;
}

onMounted(() => {
    // Initialize by loading the first batch of books
    loadMoreBooks();
});
